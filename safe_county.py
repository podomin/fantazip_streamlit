...
st.markdown("- 거주중심지역 : 안전함 검증 완료 + 다양한 가구가 거주하는 지역")
st.markdown("이렇게 두 그룹으로 나눠서 안전한 동네 추출 및 특징 파악하기")
...
st.markdown("🕵️‍♀️ : 이제 거주중심지역인 성동구에서 cctv, 유흥주점 비율을 활용해 안전한 동네를 추려보겠습니다.")
...
with st.expander(label=f"🟡 **도선동**"): 
    ...
    df= pd.DataFrame({
                    ...
                    '70~100': '💜'}, index=['범례 색상'])
    st.dataframe(df)
    st.image("./data/도선동_인구연령.png", width=700)
...
with st.expander(label=f"**🔴 홍익동  VS 🟢 상왕십리동, 하왕십리동**"): 
    ...
    st.write("🕵️‍♀️ : 하왕십리동의 인구 수가 가장 높다는 사실을 확인할 수 있습니다. ")
    ...
    st.image("./data/성동구_인구연령.png", width=800)
