...
st.write(" = 2030여성 사회초년생 1인가구에게 안전한 동네 찾기")
st.info(f"**<'안전한 동네' 페이지 분석 목적>**")
st.write("'안전한 자치구' 페이지에서는 분석과 검증을 통해 범죄 발생과 관련된 중요한 요소(cctv, 유흥주점 비율)를 발견했습니다. \
    해당 페이지에서는 이 요소들을 활용해 본래의 분석 목적인 1인가구에게 안전한 동네를 찾아보려고 합니다.")
st.write("그리고 안전한 동네와 위험한 동네의 차이점을 살펴보기 위해 인구 및 인구 연령 비율, 거주(cctv, 가로등) 및 생활(유흥주점, 경찰서, 공원 등) 환경 조건을 나타내는 데이터를 사용하여\
    안전한 동네의 특징에 대한 정보를 추출해 공유하고자 합니다.")
st.info(f"**<한계 및 대안>**")
st.write("이 과정에서 결과 추출 과정에서 어려움이 있었는데요. 분석을 통해 뽑은 안전한 자치구가 1인가구 거주 지역에 속하지 않는다는 문제점을 발견했습니다.\
    그리하여 1인가구 밀집 지역도 추가로 함께 분석하기로 결정하였습니다. ")
...
st.markdown("관악구 vs 강서구 둘 중 어디가 2030 사회초년생이 더 많이 거주하고 있을까요? 데이터로 확인해보시죠.")
with st.expander(label=f" 🔍 **관악구 vs 강서구**"): 
    ...
    st.markdown("🕵️‍♀️ : 저희의 추측대로 강서구에 취업 인구 수가 더 많은 것을 알 수 있습니다. ")
...
st.info("🕵️‍♀️ 미션 = 2030 사회초년생들이 자취하기 적합한 '1인가구중심지구' & 다양한 가구가 거주하고 있는 거주중심지역 \
...
st.markdown("🕵️‍♀️ : 먼저 안전한 자치구이면서 거주중심지역인 성동구에서 cctv, 유흥주점 비율을 활용해 안전한 동네를 추려보겠습니다.")
...
st.markdown("🕵️‍♀️ : 그래프를 살펴보니 거주중심지역인 성동구에서 상왕십리동과 하왕십리동이 안전한 동네로, 홍익동이 위험한 동네로 나왔습니다.  \
        그리고 유독 유흥주점이 이상치 수준으로 많은 도선동은 번외로 따로 살펴보겠습니다. ")
st.markdown("##### < 결과 >")
